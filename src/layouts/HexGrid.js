...
class HexGrid extends Component {
  ...
  var hexCountHorizontal = 20;
  var hexCountVertical = 20;

  var calculatePixelCoordinates = (baseVector, hexSize, axialXCoord, axialYCoord) => {
    return {
  ...
  var getOptimalSize = (widthPixels, heightPixels, countHorizontal, countVertical) => {
  ...
  var setupHexPositionsRadial = (widthPixels, heightPixels, countHorizontal, countVertical) => {
  ...
  var hexPositions = setupHexPositionsRadial(
    ...
    var hexGridContents = _.map(hexPositions, function(hexRow, index) {
      var rowElements = _.map(hexRow, function(hexData) {
        var hexKey = hexData.keyName;
        return (
          <HexTile color={hexData.color} key={ hexKey } size={hexData.size} centre={hexData.pixelCoordinates}></HexTile>
        );
      });
